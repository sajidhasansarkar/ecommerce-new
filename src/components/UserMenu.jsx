...
export default function UserMenu() {
  ...
  if (!user) {
    return (
      <Link to="/login" aria-label="লগইন" className="text-ink/70 hover:text-clay transition-colors">
        <User size={20} />
      </Link>
    )
  }
  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        onClick={() => setOpen(o => !o)}
        className="flex items-center gap-1.5 text-ink/70 hover:text-clay transition-colors"
      ...
      >
        {user.avatar ? (
          ...
          <div className="w-7 h-7 rounded-full bg-clay/20 flex items-center justify-center">
            <span className="text-xs font-semibold text-clay">{user.name?.charAt(0)?.toUpperCase()}</span>
          </div>
        )}
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-56 bg-sand border border-stone-dark rounded-xl shadow-lg z-50 overflow-hidden">
          ...
          <div className="px-4 py-3 border-b border-stone-dark">
            ...
            {user.role === 'admin' && (
              <span className="inline-block mt-1 text-[10px] font-semibold uppercase tracking-wide bg-clay/15 text-clay px-2 py-0.5 rounded-full">Admin</span>
            )}
          </div>
          ...
          <div className="py-1">
            ...
            >
              <Settings size={15} /> প্রোফাইল ও তথ্য
            </Link>
            ...
            {user.role === 'admin' && (
              <Link
                ...
                className="flex items-center gap-3 px-4 py-2.5 text-sm text-ink/80 hover:bg-stone hover:text-ink transition-colors"
              >
                <LayoutDashboard size={15} /> অ্যাডমিন প্যানেল
              </Link>
            )}
